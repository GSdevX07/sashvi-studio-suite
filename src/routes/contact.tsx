...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Get in touch with Sashvi Studio for orders, styling and replacements.",
      },
    ],
  }),
...
function ContactPage() {
  ...
  const handleSend = () => {
    const whatsappMessage = `Hi Sashvi Studio,\n\nName: ${name}\nContact: ${contact}\n\nMessage: ${message}`;
    ...
    <Layout>
      <section className="container-luxe pt-16 pb-10 md:pt-24">
        ...
        <p className="mt-3 max-w-xl text-muted-foreground">
          For enquiries, styling help, custom requests, or replacements — reach us anytime. We
      ...
      <section className="container-luxe grid gap-6 pb-20 md:grid-cols-3">
        {[
          ...
          },
          {
            ...
            label: "WhatsApp",
            value: BRAND.phone,
            href: waLink("Hi Sashvi Studio, I have an enquiry."),
          },
          {
            icon: Instagram,
            label: "Instagram",
      ...
      <section className="container-luxe pb-24">
        <div className="rounded-[1.5rem] border border-accent/30 bg-accent/10 p-8 md:p-12">
          ...
          <p className="mt-4 max-w-2xl text-foreground/80 leading-relaxed">
            For any issues with your order, please send a clear video proof of the product within 48
            hours of delivery to our WhatsApp number{" "}
            <a
              className="font-medium text-accent underline-offset-4 hover:underline"
              href={waLink("Hi, raising a replacement request with video proof.")}
              target="_blank"
              rel="noreferrer"
            >
              {BRAND.phone}
        ...
        <div className="grid gap-10 md:grid-cols-2">
          <form className="space-y-4 rounded-2xl border border-border bg-card p-8">
            ...
            <button
              type="button"
              onClick={handleSend}
              className="inline-flex items-center gap-2 rounded-full bg-foreground px-6 py-3 text-sm font-medium uppercase tracking-widest text-background hover:bg-accent hover:text-accent-foreground"
            >
          ...
          <div className="rounded-2xl border border-border bg-secondary/50 p-8">
            ...
            <div className="mt-4 flex items-start gap-3">
              ...
              <p className="text-foreground/80 leading-relaxed">
                Sashvi Studio · By appointment only.
                <br />
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                <Instagram className="h-4 w-4 text-accent" /> {BRAND.instagramHandle}
              </div>
            </div>
          </div>
        </div>
